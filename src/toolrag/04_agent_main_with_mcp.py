...
def validation_node(state: AgentState) -> AgentState:
    ...
    if execution_success:
        ...
        logger.warning("Low confidence selection → requesting user feedback")
...
def user_feedback_node(state: AgentState) -> AgentState:
    ...
    """
    logger.info("STEP 5.7: Requesting user feedback (low confidence)")
...
def run_agent(
    ...
    logger.info("Loaded {} MCP tools from server {}", len(mcp_tool_map), os.getenv("MCP_SERVER_URL"))
