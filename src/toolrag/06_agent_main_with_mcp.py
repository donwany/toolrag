...
If none of the tools does not correspond to any of the available tools with confidence = 0.0. Do not try answering the query, just retry.

Respond with ONLY a JSON object:
{{
    "reasoning": "your reasoning here",
    "selected_tool_id": "MUST BE ONE OR MORE OF THE FOLLOWING TOOL IDs BASED ON THE USER'S QUERY: {', '.join(valid_tool_ids)}",
    "confidence": 0.85
}}
"""
...
def user_feedback_node(state: AgentState) -> AgentState:
    ...
    logger.info("Awaiting user feedback via stdin: expecting confirm/select/clarify:/new:")
